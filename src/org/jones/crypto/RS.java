...
final class RS {
    ...
    static String encode(long plain) {
        ...
        int[] plain_string_10 = new int[RS.base_10_length];
        for (int i = 0; i < length; i++) {
            plain_string_10[i] = (int)plain_string.charAt(i) - (int)'0';
        }

        int codeword_length = 0;
        int[] codeword = new int[RS.initial_codeword.length];
        ...
        int[] p = {0, 0, 0, 0};
        for (int i = RS.base_32_length - 1; i >= 0; i--) {
            final int fb = codeword[i] ^ p[3];
            p[3] = p[2] ^ RS.gmult(30, fb);
            p[2] = p[1] ^ RS.gmult(6, fb);
            p[1] = p[0] ^ RS.gmult(9, fb);
            p[0] =        RS.gmult(17, fb);
        }

        System.arraycopy(p, 0, codeword, RS.base_32_length, RS.initial_codeword.length - RS.base_32_length);

        StringBuilder cypher_string_builder = new StringBuilder();
        for (int i = 0; i < 17; i++) {
            final int codework_index = RS.codeword_map[i];
            final int alphabet_index = codeword[codework_index];
            cypher_string_builder.append(RS.alphabet.charAt(alphabet_index));
    ...
    static long decode(String cypher_string) throws DecodeException {
        ...
        int[] codeword = new int[RS.initial_codeword.length];
        System.arraycopy(RS.initial_codeword, 0, codeword, 0, RS.initial_codeword.length);

        int codeword_length = 0;
        for (int i = 0; i < cypher_string.length(); i++) {
            int position_in_alphabet = RS.alphabet.indexOf(cypher_string.charAt(i));

            if (position_in_alphabet <= -1 || position_in_alphabet > RS.alphabet.length()) {
                continue;
            }
            ...
            int codework_index = RS.codeword_map[codeword_length];
            codeword[codework_index] = position_in_alphabet;
            codeword_length += 1;
        }

        if (codeword_length == 17 && !RS.is_codeword_valid(codeword) || codeword_length != 17) {
            throw new CodewordInvalidException();
        }

        int length = RS.base_32_length;
    ...
    private static int gmult(int a, int b) {
        ...
        int idx = (RS.glog[a] + RS.glog[b]) % 31;
    ...
    private static boolean is_codeword_valid(int[] codeword) {
        ...
        for (int i = 1; i < 5; i++) {
            ...
            for (int j = 0; j < 31; j++) {
                ...
                t ^= RS.gmult(codeword[pos], RS.gexp[(i * j) % 31]);
            }
